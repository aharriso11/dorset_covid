...
unvaxxed_plot <- ggplot(data = subset(vaccs_percentage_long, event == "First")) +
  ...
  labs(caption = paste("Data from UK Health Security Agency / https://coronavirus.data.gov.uk. Plotted", Sys.time(), sep = " "), subtitle = "Percentage of people aged 12 and over in a MSOA that have not yet received a first covid vaccination, plotted against the MSOA's deprivation score.") +
  theme_base() +
  theme(
